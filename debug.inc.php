 ...
 * @link     https://gitlab.acu.edu.au/php/apps.acu.edu.au/form_admin
 */
...
if (!function_exists('debug')) :
        ...
        if (DEBUG__STATE === true || !defined('DEBUG__DISCONTINUE_'.$debug_line)) {
            ...
            for ($a = 0; $a < func_num_args(); $a += 1) {
                ...
                case 'string':
                    ...
                    case 'constant':
                    case 'constants':
                        debug__get_const_name(
                            $var_value, $var_name, $current_constants, true
                        );
                        break;

                    default:
                        $reg = '/^(max[- _]?times|meta[- _]?level) ?(?:= ?)?([0-9]*)$/';
    ...
    function debug_conf()
    {
        if (!defined('DEBUG__CONF')) {
            ...
            if (version_compare(phpversion(), '5.1.0') == 1
                ...
                && $config['time_adjust'] < 780
                && is_int($config['time_adjust'] / 15)
            ) {
            ...
            foreach ($enviro_tmp as $setting) {
                ...
                default:
                    ...
                    } elseif (preg_match($reg, $setting, $matches)) {
    ...
    /**
     * Move the server time forward or backward as
...
';
                ...
                if (!empty($matches)) {
                    foreach ($matches as $conf) {
                        if (!empty($conf)) {
                            ...
                            case 'log_file':
                                if (is_file($conf[2]) && is_readable($conf[2])
                                    && is_writable($conf[2])
                                ) {
                                    $config[$key] = $conf[2];
                                }
     ...
     *                type (if supplied)
     */
    function debug__die_with_info(
        $message = '',
        $dead_input = DEBUG__DIE_WITH_INFO__NO_VAR,
        $depth = 1
    ) {
        if (!is_string($message)) {
            $output_msg = 'debug__die_with_info() expects first parameter '.
    ...
    function debug__help(&$arr)
    {
        ...
        if (is_file($helpFile)) {
            ...
            debug_debug('Unable to find debug help file in '.$helpFile);
        }
    }
    ...
    function debug__get_var_name($var_value, &$var_name, $current_globals)
    {
        ...
        foreach ($current_globals as $globals_var_name => $globals_var_value) {
            if ($globals_var_value === $var_value) {
                ...
                debug__get_var_name_from_array(
                    $tmp_name, $var_value, $globals_var_name, $globals_var_value
                );
            }

            if ($tmp_name !== ''
                && debug__no_vname_already($tmp_name, $var_name) === true
            ) {
                $var_name['count'] += 1;
                if ($var_name['count'] > 1) {
     ...
     * @return void
     */
    function debug__get_const_name(

 ...
 *	if the script was being called from apps or testapps it would
 *	return '192.148.223.212'.
 *      if it was being called from dev1 or localhost it would return
 *	'localhost'.
 *
 * @const CHECK_HOST__SERVER_HTTP_HOST string to identify host set
 *	the first time the function is called.
 *
 * @param $param_1 mixed 
 *	if string, (for backwards compatibility)
 *		   value to be used if running on apps.acu.edu.au or
 *		   blogs.acu.edu.au server
 *	if array   if host matches array key, array key value is
 *		   returned.
 * 		   if array key is not matched but check_host_name()
 *		   thinks $param_1 specifies domain values, the first
 ...
 *		if host is not apps or testapps or dev1, 'bar bar' is
 *		returned
 *
 * @param $param_2 mixed  value to be used running on testapps.acu.edu.au
 *			  or studentblogs.acu.edu.au server
 * @param $param_3 mixed  correct value to be used running on
 *			  dev1.acu.edu.au or dev-blogs.acu.edu.au
 *			  server
 * @param $param_4 mixed  correct value to be used running on
 *			  localhost or dev.student.blogs.acu.edu.au
 *			  server
 * @param $param_5 mixed  correct value to be used running on an
 *			  unknown server
 *
 * @return mixed
 */
function check_host( $param_1 = '' , $param_2 = '' , $param_3 = '' , $param_4 = '' , $param_5 = '' )
{
	...
	if(!defined('CHECK_HOST__SERVER_HTTP_HOST'))
	{
		...
		{
			if(preg_match('/^(?:(localhost)|(apps|testapps|dev1|(?:(?:dev\.?)?student\.?)?(dev[.-]?)?blogs)\.acu\.edu\.au)$/i',$_SERVER['HOSTNAME'],$matches))
			{
				$matches[2] = isset($matches[2])?$matches[2]:'';
				$host = $matches[1].$matches[2];
			};
